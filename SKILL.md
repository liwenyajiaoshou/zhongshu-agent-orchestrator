...
13. **Codex Model-Thread Affinity**：本原则仅适用于 Codex。Codex 线程默认保持模型稳定；任务变复杂时先判断上下文质量与当前模型能力。若当前模型能力仍足够且上下文价值高，优先提高推理等级；只有确认模型能力不足时才升级模型。已有实质施工上下文时，模型切换默认通过新 Codex 线程完成。
14. **Cached Context Is Not Free Context**：本原则仅适用于 Codex。缓存上下文可降低部分重复输入成本，但不能消除 stale state、旧错误路径、已失效假设和注意力竞争。不得仅因 cached input 较高就无限延长 Codex 线程。
15. **Token Is a Signal, Not a Threshold**：本原则仅适用于 Codex。Token 量级只作为 Context Saturation 的辅助信号，不设置机械 Token 硬阈值；换线程应结合当前任务对历史上下文的依赖、已关闭问题、失效假设、blocker 转换和重复调查迹象综合判断。
16. **Finish Local Value, Then Compress**：本原则仅适用于 Codex。当前高价值局部任务仍强依赖现有上下文时，不因 Token 压力强行中断；一旦形成稳定 PASS / OFFLINE_PASS / 明确 blocker，应立即评估压缩交接并切换 Codex 线程。
17. **Execution Form Is Not Risk**：PowerShell、Shell、Python、CLI 只是执行形式，不是风险等级。中枢必须依据真实副作用、授权边界与执行环境决定由 Codex 还是 Owner 执行。
